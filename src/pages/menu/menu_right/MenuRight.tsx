...
export default function MenuRight(props: Props) {
    ...
    return (
        <>
            {props.size === "mobile" && props.openFilter || props.size === "tablet" && props.openFilter ? <div onClick={() => props.setOpenFilter(false)} className="overplay"></div> : ''}
            <div className={`menu_right ${props.size}`}>
                <div className="right_top">
                    ...
                    <div className="category">
                        {categoryData.map((category) => {
                            return (
                                <Category key={category.id} category={category} setCategory={props.setCategory} size={props.size} />
                            )
                        })}
                    </div>
                </div>
                ...
                <div className="right_mid">
                    ...
                    <div className="option">
                        {props.size === "desktop" ? <p>Sort By:</p> : <button className="btn_collapse" onClick={() => props.setOpenFilter(!props.openFilter)}><BiMenuAltLeft /></button>}
                        <div className="collapse_right">
                            <button className="btn_collapse" onClick={() => checkHeight(elRef)}>{props.size === "desktop" ? "Popular" : ''}<AiOutlineDown /></button>
                ...
                <div className="right_bottom">
                    {props.filterMenu.length !== 0 ? props.filterMenu.map((menuData) => {
                        return (
